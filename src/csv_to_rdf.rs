...
const PREFIXES:&str = "@prefix xsd: <http://www.w3.org/2001/XMLSchema#> . @prefix baseUrl: <http://example.data/event/> . ";

pub fn csv_to_rdf(data: &[u8]) -> Result<Vec<u8>, Box<dyn Error>>
{
    let mut result: Vec<u8> = Vec::new();
    result.extend_from_slice(PREFIXES.as_bytes());
    ...
    for row in rdr.records() {
        ...
        let mut s_row = format!("baseUrl:{} ", uuid);
